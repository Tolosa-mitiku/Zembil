import { useState } from 'react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import { businessInfoSchema, socialMediaSchema, BusinessInfoFormData, SocialMediaFormData } from '../validation/schemas';
import { SellerProfile } from '../api/profileApi';
import ImageUpload from '@/shared/components/ImageUpload';
import Input from '@/shared/components/Input';
import Button from '@/shared/components/Button';
import {
  BuildingStorefrontIcon,
  HashtagIcon,
  IdentificationIcon,
  MapPinIcon,
  DocumentTextIcon,
  TagIcon,
  PencilIcon,
  CheckIcon,
  XMarkIcon,
  GlobeAltIcon,
} from '@heroicons/react/24/outline';
import toast from 'react-hot-toast';

interface BusinessTabProps {
  profile: SellerProfile;
  onUpdate: (data: any) => Promise<any>;
}

const BusinessTab = ({ profile, onUpdate }: BusinessTabProps) => {
  const [isEditingBusiness, setIsEditingBusiness] = useState(false);
  const [isEditingSocial, setIsEditingSocial] = useState(false);

  const {
    register: registerBusiness,
    handleSubmit: handleSubmitBusiness,
    formState: { errors: businessErrors, isSubmitting: isSubmittingBusiness },
    reset: resetBusiness,
  } = useForm<BusinessInfoFormData>({
    resolver: zodResolver(businessInfoSchema),
    values: {
      businessName: profile.businessName || '',
      businessRegistrationNumber: profile.businessRegistrationNumber || '',
      taxId: profile.taxId || '',
      businessAddress: profile.businessAddress || '',
      businessDescription: profile.businessDescription || '',
      businessCategory: profile.businessCategory || '',
    },
  });

  const {
    register: registerSocial,
    handleSubmit: handleSubmitSocial,
    formState: { errors: socialErrors, isSubmitting: isSubmittingSocial },
    reset: resetSocial,
  } = useForm<SocialMediaFormData>({
    resolver: zodResolver(socialMediaSchema),
    values: {
      instagram: profile.socialMedia?.instagram || '',
      facebook: profile.socialMedia?.facebook || '',
      twitter: profile.socialMedia?.twitter || '',
      website: profile.socialMedia?.website || '',
    },
  });

  const onSubmitBusiness = async (data: BusinessInfoFormData) => {
    try {
      await onUpdate(data).unwrap();
      toast.success('Business information updated successfully!');
      setIsEditingBusiness(false);
    } catch (error: any) {
      toast.error(error?.data?.message || 'Failed to update business information');
    }
  };

  const onSubmitSocial = async (data: SocialMediaFormData) => {
    try {
      await onUpdate({ socialMedia: data }).unwrap();
      toast.success('Social media links updated successfully!');
      setIsEditingSocial(false);
    } catch (error: any) {
      toast.error(error?.data?.message || 'Failed to update social media');
    }
  };

  const handleCancelBusiness = () => {
    resetBusiness();
    setIsEditingBusiness(false);
  };

  const handleCancelSocial = () => {
    resetSocial();
    setIsEditingSocial(false);
  };

  const businessCategories = [
    'Electronics',
    'Fashion & Apparel',
    'Home & Garden',
    'Beauty & Personal Care',
    'Sports & Outdoors',
    'Books & Media',
    'Toys & Games',
    'Food & Beverage',
    'Health & Wellness',
    'Automotive',
    'Other',
  ];

  return (
    <div className="space-y-6">
      {/* Business Logo */}
      <div className="bg-white rounded-xl border border-grey-200 p-6">
        <h3 className="text-lg font-semibold text-grey-900 mb-4">Business Logo</h3>
        <div className="flex items-start gap-6">
          <ImageUpload
            value={profile.businessLogo}
            onChange={() => {}}
            onUpload={async (file) => {
              const formData = new FormData();
              formData.append('businessLogo', file);
              // TODO: Implement business logo upload endpoint
              return profile.businessLogo || '';
            }}
            shape="square"
            maxSize={2}
            description="PNG, JPG up to 2MB"
            label=""
          />
          <div className="flex-1">
            <h4 className="font-medium text-grey-900 mb-1">Upload Business Logo</h4>
            <p className="text-sm text-grey-500 mb-4">
              This will be displayed on your store page and in the marketplace. Recommended size: 400x400px
            </p>
            <ul className="text-xs text-grey-500 space-y-1">
              <li>• Use a square image for best results</li>
              <li>• Transparent background recommended</li>
              <li>• High resolution for crisp display</li>
            </ul>
          </div>
        </div>
      </div>

      {/* Business Information */}
      <div className="bg-white rounded-xl border border-grey-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-grey-900">Business Information</h3>
          {!isEditingBusiness && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsEditingBusiness(true)}
              icon={<PencilIcon className="w-4 h-4" />}
            >
              Edit
            </Button>
          )}
        </div>

        <form onSubmit={handleSubmitBusiness(onSubmitBusiness)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label="Business Name"
              {...registerBusiness('businessName')}
              error={businessErrors.businessName?.message}
              icon={<BuildingStorefrontIcon className="w-5 h-5" />}
              placeholder="Your business name"
              disabled={!isEditingBusiness}
            />

            <Input
              label="Registration Number"
              {...registerBusiness('businessRegistrationNumber')}
              error={businessErrors.businessRegistrationNumber?.message}
              icon={<HashtagIcon className="w-5 h-5" />}
              placeholder="Business registration number"
              disabled={!isEditingBusiness}
            />

            <Input
              label="Tax ID / VAT Number"
              {...registerBusiness('taxId')}
              error={businessErrors.taxId?.message}
              icon={<IdentificationIcon className="w-5 h-5" />}
              placeholder="Tax identification number"
              disabled={!isEditingBusiness}
            />

            <div>
              <label className="block text-sm font-medium text-grey-700 mb-2">
                Business Category
              </label>
              <div className="relative">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <TagIcon className="w-5 h-5 text-grey-400" />
                </div>
                <select
                  {...registerBusiness('businessCategory')}
                  disabled={!isEditingBusiness}
                  className={`w-full pl-10 pr-4 py-3 border rounded-lg transition-colors ${
                    isEditingBusiness
                      ? 'border-grey-200 focus:border-gold focus:ring-2 focus:ring-gold-pale'
                      : 'bg-grey-50 border-grey-100 text-grey-700 cursor-not-allowed'
                  }`}
                >
                  <option value="">Select a category</option>
                  {businessCategories.map((category) => (
                    <option key={category} value={category}>
                      {category}
                    </option>
                  ))}
                </select>
              </div>
              {businessErrors.businessCategory && (
                <p className="text-sm text-error mt-1">{businessErrors.businessCategory.message}</p>
              )}
            </div>

            <div className="md:col-span-2">
              <Input
                label="Business Address"
                {...registerBusiness('businessAddress')}
                error={businessErrors.businessAddress?.message}
                icon={<MapPinIcon className="w-5 h-5" />}
                placeholder="Full business address"
                disabled={!isEditingBusiness}
              />
            </div>

            <div className="md:col-span-2">
              <label className="block text-sm font-medium text-grey-700 mb-2">
                Business Description
              </label>
              <textarea
                {...registerBusiness('businessDescription')}
                rows={4}
                placeholder="Tell customers about your business..."
                disabled={!isEditingBusiness}
                className={`w-full px-4 py-3 border rounded-lg transition-colors ${
                  isEditingBusiness
                    ? 'border-grey-200 focus:border-gold focus:ring-2 focus:ring-gold-pale'
                    : 'bg-grey-50 border-grey-100 text-grey-700 cursor-not-allowed'
                }`}
              />
              {businessErrors.businessDescription && (
                <p className="text-sm text-error mt-1">{businessErrors.businessDescription.message}</p>
              )}
            </div>
          </div>

          {isEditingBusiness && (
            <div className="flex gap-3 mt-6 pt-6 border-t border-grey-100">
              <Button
                type="submit"
                variant="primary"
                size="md"
                disabled={isSubmittingBusiness}
                icon={<CheckIcon className="w-4 h-4" />}
              >
                {isSubmittingBusiness ? 'Saving...' : 'Save Changes'}
              </Button>
              <Button
                type="button"
                variant="ghost"
                size="md"
                onClick={handleCancelBusiness}
                disabled={isSubmittingBusiness}
                icon={<XMarkIcon className="w-4 h-4" />}
              >
                Cancel
              </Button>
            </div>
          )}
        </form>
      </div>

      {/* Social Media Links */}
      <div className="bg-white rounded-xl border border-grey-200 p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-lg font-semibold text-grey-900">Social Media</h3>
          {!isEditingSocial && (
            <Button
              variant="ghost"
              size="sm"
              onClick={() => setIsEditingSocial(true)}
              icon={<PencilIcon className="w-4 h-4" />}
            >
              Edit
            </Button>
          )}
        </div>

        <form onSubmit={handleSubmitSocial(onSubmitSocial)}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Input
              label="Instagram"
              {...registerSocial('instagram')}
              error={socialErrors.instagram?.message}
              icon={
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                </svg>
              }
              placeholder="https://instagram.com/yourbusiness"
              disabled={!isEditingSocial}
            />

            <Input
              label="Facebook"
              {...registerSocial('facebook')}
              error={socialErrors.facebook?.message}
              icon={
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              }
              placeholder="https://facebook.com/yourbusiness"
              disabled={!isEditingSocial}
            />

            <Input
              label="Twitter"
              {...registerSocial('twitter')}
              error={socialErrors.twitter?.message}
              icon={
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" />
                </svg>
              }
              placeholder="https://twitter.com/yourbusiness"
              disabled={!isEditingSocial}
            />

            <Input
              label="Website"
              {...registerSocial('website')}
              error={socialErrors.website?.message}
              icon={<GlobeAltIcon className="w-5 h-5" />}
              placeholder="https://yourbusiness.com"
              disabled={!isEditingSocial}
            />
          </div>

          {isEditingSocial && (
            <div className="flex gap-3 mt-6 pt-6 border-t border-grey-100">
              <Button
                type="submit"
                variant="primary"
                size="md"
                disabled={isSubmittingSocial}
                icon={<CheckIcon className="w-4 h-4" />}
              >
                {isSubmittingSocial ? 'Saving...' : 'Save Changes'}
              </Button>
              <Button
                type="button"
                variant="ghost"
                size="md"
                onClick={handleCancelSocial}
                disabled={isSubmittingSocial}
                icon={<XMarkIcon className="w-4 h-4" />}
              >
                Cancel
              </Button>
            </div>
          )}
        </form>
      </div>
    </div>
  );
};

export default BusinessTab;

